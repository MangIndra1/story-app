    ...
    this.#stories.forEach(story => {
      if (story.lat != null && story.lon != null && !isNaN(story.lat) && !isNaN(story.lon)) {
        try {
          ...
          console.error(`Gagal membuat marker untuk cerita ID ${story.id}:`, error);
        }
      } else {
        console.warn(`Data lokasi tidak valid untuk cerita ID ${story.id}: lat=${story.lat}, lon=${story.lon}`);
      }
    });
  }
